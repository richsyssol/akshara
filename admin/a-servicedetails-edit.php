...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $sql = "SELECT * FROM  ser_details WHERE order_id = $product_id";
    ...
    $image_sql = "SELECT * FROM sub_otherser_serv_image WHERE sub_serv_id = $product_id";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Update serv details (excluding images first)
    if ($id > 0) {
        $update_sql = "UPDATE  ser_details SET name = '$name', description = '$description' WHERE order_id = $id";
    ...
    if (!empty($_FILES['images']['name'][0])) {
        ...
        foreach ($_FILES['images']['name'] as $key => $image_name) {
            ...
            if (move_uploaded_file($_FILES["images"]["tmp_name"][$key], $target_file)) {
                ...
                $insert_image_sql = "INSERT INTO sub_otherser_serv_image (sub_serv_id, image_path) 
