...
<?php 
    ...
    if(isset($_POST['submit'])) {
        ...
        $upload_dir = 'uploads/about-us/'; // Ensure this directory exists and has write permissions
        $upload_file = $upload_dir . basename($image);

        if (move_uploaded_file($image_tmp, $upload_file)) {
            // Insert the file path into the database
            $sql = "INSERT INTO `about_us`(`id`, `image`, `text`) VALUES (NULL, '$upload_file', '$text')";
            $result = mysqli_query($conn, $sql);
            
            if($result){
                // Use JavaScript to redirect
                echo '<script>window.location.href = "about-us";</script>';
                exit();
