...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    $sql = "SELECT * FROM event_detail WHERE order_id = $product_id";
    ...
    $image_sql = "SELECT * FROM sub_otherser_event_images WHERE sub_event_id = $product_id";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Update event details (excluding images first)
    if ($id > 0) {
        $update_sql = "UPDATE event_detail SET name = '$name', description = '$description' WHERE order_id = $id";
    ...
    if (!empty($_FILES['images']['name'][0])) {
        ...
        foreach ($_FILES['images']['name'] as $key => $image_name) {
            ...
            if (move_uploaded_file($_FILES["images"]["tmp_name"][$key], $target_file)) {
                ...
                $insert_image_sql = "INSERT INTO sub_otherser_event_images (sub_event_id, image_path) 
    ...
    echo '<script>window.location.href = "a-productdetails.php";</script>';
    exit();
}
}
?>
